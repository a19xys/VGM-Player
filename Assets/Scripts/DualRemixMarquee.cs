...
/// <summary>
/// Marquee dual y sincronizado para dos TMP (versi�n con baseline fijo).
/// - Captura la posici�n de inicio REAL (baseline) una sola vez por r�tulo.
/// - En cada reset de canci�n, fuerza ambos a su baseline antes de medir.
/// - M�quina de estados robusta: PauseStart -> Left -> PauseEnd -> Right.
/// </summary>
public class DualRemixMarquee : MonoBehaviour
{
    ...
    [Tooltip("Viewport visible (soft mask). Si se deja vac�o, se usa el padre de textA.")]
    public RectTransform viewport;
    [Tooltip("Opcional: pausa el scroll cuando la m�sica est� en pausa.")]
    ...
    private bool active;        // si el ciclo est� en marcha

    void OnEnable()
    {
        // Al reactivar, medimos en el pr�ximo Update.
        initialized = false;
    }
    ...
    /// <summary>
    /// Llamar con la pantalla cubierta al cambiar de canci�n/remix.
    /// Fuerza ambos textos a su baseline inicial y reinicia ciclo.
    /// </summary>
    public void ResetAndStart()
    {
        active = true;
        initialized = false; // provocamos el �hard reset� en el pr�ximo Update
    }
    ...
    private void RecalculateMetricsInternal()
    {
        ...
        // Colocar ambos al �inicio l�gico� (startX == baseline)
        a.SnapToStart();
        b.SnapToStart();
    }
    ...
    private class Line
    {
        ...
        // baseline fijo (la posici�n de layout original, capturada 1 vez)
        ...
        /// <summary>
        /// Captura la posici�n de inicio de layout (baseline) una sola vez.
        /// Si forceSnap es true y ya hab�a baseline, NO se sobreescribe;
        /// s�lo forzamos el Rect a ese baseline al llamar a SnapToBaseline().
        /// </summary>
